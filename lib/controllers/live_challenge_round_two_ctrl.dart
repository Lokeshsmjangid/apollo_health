...
class LiveChallengeRoundTwoCtrl extends GetxController{

  // live challeng waiting screen time round -2
  int round2RemainingSeconds = 0; // initial countdown value
  Timer? waitingTimer;
  ...
  void onInit() {
    ...
    if(Get.arguments!=null) {
      ...
      if(questionsApi.isNotEmpty){
        Future.microtask((){
          round2RemainingSeconds = questionsApi.length*10;
          update();
          startTimer();
        });
      }
  ...
  void startTimer() {
    waitingTimer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (round2RemainingSeconds > 0) {
        round2RemainingSeconds--;
        update();
      } else {
        timer.cancel();
      }

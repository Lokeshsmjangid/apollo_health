...
class LiveChallengeQuizScreen extends StatefulWidget {
  const LiveChallengeQuizScreen({super.key});
...
class _LiveChallengeQuizScreenState extends State<LiveChallengeQuizScreen> {
  ...
  Widget _buildFrontCardNew(SoloPlayQuestion q, int qIndex, LiveChallengeQuizCtrl logic, int currentIndex) {
    return Container(
                    ...
                    child: CountdownTimerWidget(
                      ...
                      onTimerEnd: (){
                        if(logic.isLastQ==true){
                          ...
                          roundWaitingTime = logic.round1RemainingSeconds;
                          Get.off(()=>LiveChallengeRoundTimerScreen(
                              livePlayId: id,
                              round: 2,
                              countParticipants: cp));

                           // waiting screen se phale ye code tha
                          /*showLoader(true);
                          changeRoundLiveChallengesApi(liveChallengeId: logic.livePlayId,round: 2).then((value){
                            showLoader(false);
                            if(value.status==true){
                              Get.offNamed(AppRoutes.liveChallengeRoundTwoScreen,
                                  arguments: {'live_play_id':
                                  id,'questions': value.data!.questions,'count_participants':cp});
                      ...
                      if (q.isAnswered) {
                        ...
                        if (qIndex < logic.questionsApi.length - 1) {
                          ...
                          changeRoundLiveChallengesApi(liveChallengeId: logic.livePlayId,round: 2).then((value){
                            showLoader(false);
                            if(value.status==true){
                              Get.offNamed(AppRoutes.liveChallengeRoundTwoScreen,arguments: {

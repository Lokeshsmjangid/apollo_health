...
class LiveChallengeRoundTwoScreen extends StatefulWidget {
  ...
  @override
  State<LiveChallengeRoundTwoScreen> createState() =>
      _LiveChallengeRoundTwoScreenState();
}

class _LiveChallengeRoundTwoScreenState extends State<LiveChallengeRoundTwoScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<LiveChallengeRoundTwoCtrl>(
        builder: (logic) {
          return SafeArea(
            bottom: false,
            child: Column(
  ...
  Widget _buildFrontCardNew(SoloPlayQuestion q, int qIndex, LiveChallengeRoundTwoCtrl logic, int currentIndex) {
    return Container(
                      ...
                      onTimerEnd: (){
                        if(logic.isLastQ==true){
                          ...
                          if(!q.isAnswered && currentIndex==qIndex){
                            submitAnswerLiveChallengeApi( // to skip question
                          ...
                          roundWaitingTime = logic.round2RemainingSeconds;
                          ...
                          /*showLoader(true); // waiting screen se phale ye tha
                          changeRoundLiveChallengesApi(liveChallengeId: logic.livePlayId,round: 3).then((value){
                            showLoader(false);
                            if(value.status==true){
                              Get.offNamed(AppRoutes.liveChallengeRoundThreeScreen,
                                  arguments: {'live_play_id':  id,'questions': value.data!.questions,'count_participants':cp});
                    ...
                    if (q.options![optIndex].isNotEmpty && !q.isAnswered) {
                      ...
                      if (q.isAnswered) {
                        ...
                        if (qIndex < logic.questionsApi.length - 1) {
                         ...
                         /* showLoader(true); // waiting screen se phale ye code tha
                          changeRoundLiveChallengesApi(liveChallengeId: logic.livePlayId,round: 3).then((value){
                            showLoader(false);
                            if(value.status==true){
                              Get.offNamed(AppRoutes.liveChallengeRoundThreeScreen,arguments: {
                                'live_play_id':  id,'questions': value.data!.questions,'count_participants':cp});

...
class LiveChallengeRoundThreeScreen extends StatefulWidget {
  const LiveChallengeRoundThreeScreen({super.key});

  @override
  State<LiveChallengeRoundThreeScreen> createState() =>
      _LiveChallengeRoundThreeScreenState();
}

class _LiveChallengeRoundThreeScreenState extends State<LiveChallengeRoundThreeScreen> {
  final GlobalKey<StackedDraggableSwiperState> swiperKey = GlobalKey<StackedDraggableSwiperState>();
  final AudioPlayer _audioPlayer = AudioPlayer();

  // live challeng waiting screen time round -3
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<LiveChallengeRoundThreeCtrl>(
        builder: (logic) {
          return SafeArea(
            bottom: false,
            child: Column(
  ...
  Widget _buildFrontCardNew(SoloPlayQuestion q, int qIndex, LiveChallengeRoundThreeCtrl logic, int currentIndex) {
    return Container(
                    ...
                    child: CountdownTimerWidget(
                      ...
                      onTimerEnd: (){
                        if(logic.isLastQ==true){
                          ...
                          roundWaitingTime = logic.round3RemainingSeconds;
                          ...
                          changeRoundLiveChallengesApi(liveChallengeId: logic.livePlayId,round: 4).then((value){
                            showLoader(false);
                            if(value.status==true){
                              Get.offNamed(AppRoutes.liveChallengeRoundFourScreen,
                                  arguments: {'live_play_id':  id,
                                    'questions': value.data!.questions,
                                    'count_participants':cp});

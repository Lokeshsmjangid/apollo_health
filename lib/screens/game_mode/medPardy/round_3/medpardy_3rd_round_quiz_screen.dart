...
import 'package:apollo/resources/Apis/api_repository/medpardy_result_repo.dart';
...
import 'package:apollo/screens/game_mode/medPardy/medparty_result_screen.dart';
...
import 'medpardy_3rd_round_quiz_ctrl.dart';

class Medpardy3rdRoundQuizScreen extends StatefulWidget {
  const Medpardy3rdRoundQuizScreen({super.key});

  @override
  State<Medpardy3rdRoundQuizScreen> createState() => _Medpardy3rdRoundQuizScreenState();
}

class _Medpardy3rdRoundQuizScreenState extends State<Medpardy3rdRoundQuizScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<Medpardy3rdRoundQuizCtrl>(
        builder: (logic) {
          return SafeArea(
            bottom: false,
            child: Column(
  ...
  Widget _buildFrontCard(SoloPlayQuestion q, int qIndex, Medpardy3rdRoundQuizCtrl logic, int currentIndex) {
                    ...
                    GestureDetector(
                      onTap: () {
                        showLeaveQuizSheet(context, () {
                          ...
                          logic.update();
                          Get.back();
                          Get.delete<Medpardy3rdRoundQuizCtrl>(force: true);
                        ...
                        onTimerEnd: (){
                          if(!q.isAnswered){
                            ...
                            if(selectedCells.length==3){
                              ...
                              logic.update();
                              showLoader(true);
                              medpardyResultApi(gameId: gameId).then((result){
                                showLoader(false);
                                if(result.status==true && result.data!=null){
                                  Get.to(()=>MedpardyResultScreen(userList: result.data??[],));
                                }
                              });
                      ...
                      onTap: () {
                        ...
                        if (q.options![optIndex].isNotEmpty && !q.isAnswered) {
                          ...
                          if (q.isAnswered) {
                            ...
                            logic.players[logic.selectedPlayerIndex!].hp = (optIndex == q.correctIndex)? (logic.players[logic.selectedPlayerIndex!].hp+logic.selectedXp!):(logic.players[logic.selectedPlayerIndex!].hp-logic.selectedXp!);
                            logic.update();
                            ...
                            if (qIndex < logic.questionsApi.length - 1) {
                              ...
                              logic.players[logic.selectedPlayerIndex!].hp = (optIndex == q.correctIndex)? (logic.players[logic.selectedPlayerIndex!].hp+logic.selectedXp!)
                                  :(logic.players[logic.selectedPlayerIndex!].hp-logic.selectedXp!);
                              logic.update();
  ...
  Widget _buildBackCard(SoloPlayQuestion q, int qIndex, Medpardy3rdRoundQuizCtrl logic, int currentIndex) {
    return SizedBox.expand(
      child: Container(
        padding: EdgeInsets.only(left: 10, top: 10, right: 10),
        decoration: BoxDecoration(
                    ...
                    children: [
                      ...
                      if(logic.currentIndex==qIndex && q.isAnswered)
                        Positioned(
                          left: 12,
                          top: 12,
                          child: CountdownTimerWidgetExplanation( // back card not used
                          key: ValueKey(currentIndex),
                          duration: 40,
                          isInSecondsMode: true,
                          shouldStop: logic.shouldStopAllTimers,

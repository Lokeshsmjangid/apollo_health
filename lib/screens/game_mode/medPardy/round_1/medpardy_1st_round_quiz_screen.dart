...
class Medpardy1stRoundQuizScreen extends StatefulWidget {
  ...
  @override
  State<Medpardy1stRoundQuizScreen> createState() => _Medpardy1stRoundQuizScreenState();
}

class _Medpardy1stRoundQuizScreenState extends State<Medpardy1stRoundQuizScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<Medpardy1stRoundQuizCtrl>(
        builder: (logic) {
          return SafeArea(
            bottom: false,
            child: Column(
  ...
  Widget _buildFrontCard(SoloPlayQuestion q, int qIndex, Medpardy1stRoundQuizCtrl logic, int currentIndex) {
                    ...
                    GestureDetector(
                      onTap: () {
                        showLeaveQuizSheet(context, () {
                          ...
                          logic.update();
                          Get.back();
                          Get.delete<Medpardy1stRoundQuizCtrl>(force: true);
                        ...
                        onTimerEnd: (){
                          ...
                          if(!q.isAnswered){
                            ...
                            if(selectedCells.length==15){
                              showLoader(true);
                              medpardyChangeRoundApi(gameId: gameId, round: 2).then((value){
                                showLoader(false);
                            ...
                            else{ Get.offNamed(AppRoutes.medpardy1stRoundScreen,arguments: {
                        ...
                        if (q.options![optIndex].isNotEmpty && !q.isAnswered) {
                          ...
                          if (q.isAnswered) {
                            ...
                            if (qIndex < logic.questionsApi.length - 1) {
                              ...
                              apolloPrint(message: "Abhi aaya");
                              logic.players[logic.selectedPlayerIndex!].hp = (optIndex == q.correctIndex)? (logic.players[logic.selectedPlayerIndex!].hp+logic.selectedXp!):(logic.players[logic.selectedPlayerIndex!].hp-0);
                              logic.update();
                              Future.delayed(Duration(milliseconds: 2000),(){ // working code
                             bool initialTime = false;
                             int playerIndex = logic.selectedPlayerIndex!;
                             int gameId = logic.gameId??-10;
                             List<Category> categories = logic.categories;
                             ...
                             if(selectedCells.length==15){
                               ...
                               Get.offNamed(AppRoutes.medpardy1stRoundScreen,arguments: {
                                 'initialTime': initialTime,
                                 'game_id': gameId,
                                 'category_list': categories,
                                 'selected_player': playerIndex==0?1:playerIndex==1?2:playerIndex==2?0:-1,
                          ...
                          medpardySubmitAnswerApi(
                              ...
                              qId: q.id,
                              selectedOption: q.selectedIndex==0?'A':q.selectedIndex==1?'B':q.selectedIndex==2?'C':q.selectedIndex==3?'D':"E",
  ...
  Widget _buildBackCard(SoloPlayQuestion q, int qIndex, Medpardy1stRoundQuizCtrl logic, int currentIndex) {
    return SizedBox.expand(
      child: Container(
        padding: EdgeInsets.only(left: 10, top: 10, right: 10),
        decoration: BoxDecoration(

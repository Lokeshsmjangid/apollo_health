...
class Medpardy1stRoundScreen extends StatefulWidget {
  ...
  Medpardy1stRoundScreen();

  @override
  State<Medpardy1stRoundScreen> createState() => _Medpardy1stRoundScreenState();
}

class _Medpardy1stRoundScreenState extends State<Medpardy1stRoundScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          GestureDetector(
            onTap: (){
              showLeaveQuizSheet(context, () {
                Get.delete<MedPardy1stRoundCtrl>(force: true);
      ...
      body: GetBuilder<MedPardy1stRoundCtrl>(builder: (logic) {
        return Container(
          decoration: BoxDecoration(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  AppButton(
                    onButtonTap: () {
                      // if (logic.selectedCol != null && logic.selectedRow != null) {
                      //   final selectedPoints = logic.board[logic.selectedCol!][logic.selectedRow!].points;
                      //   final currentPlayer = logic.players[logic.selectedPlayerIndex??0].name;
                      //   print('Selected $selectedPoints points by $currentPlayer');
                      //   print('Selected Column and row ${logic.selectedCol}<-> ${logic.selectedRow}');
                      // }
                      if (logic.selectedCell!=null) {
                        ...
                        element.row == logic.selectedCell?.row && element.col == logic.selectedCell?.col)) {
                          ...
                          showLoader(true);
                          startMedpardyApi(gameId: gameID, round: 1,categoryId: catID,playerIndex: 0, playerName: players[0].name, xp: selectedPoints).then((value){
                            showLoader(false);
                            if(value.status==true && value.data!=null){
                              Get.offNamed(AppRoutes.medpardy1stRoundQuizScreen, arguments: {
                                'initialTime': false,
                                'game_id': gameID,
                                'round': 1,
                      ...
                      // Get.offNamed(AppRoutes.quizMedpardyScreen, arguments: {
                      //   'round': 1,
                      //   'fromPlayer': logic.players[logic.selectedPlayerIndex??0].name
                      // });
                    },
                    buttonText: 'Play',

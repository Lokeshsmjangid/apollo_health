...
Future<SoloPlayResultModel> soloPlayResultApi({gameId}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return SoloPlayResultModel.fromJson({});
  }
  try{
    String url = ApiUrls.submitResultUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return SoloPlayResultModel.fromJson(data);
    } else {
  ...
  return SoloPlayResultModel.fromJson({}); // please add try catch to use this

...
Future<GroupPlayResultModel> groupPlayResultApi({gameId}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return GroupPlayResultModel.fromJson({});
  }
  try{
    String url = ApiUrls.submitResultGroupUrl;
    final Map<String, dynamic> map = {
      'group_game_id':gameId,
      'game_complete':1, // 1-complete 2 - still playing(not using for now)
    };
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return GroupPlayResultModel.fromJson(data);
    } else {
  ...
  return GroupPlayResultModel.fromJson({}); // please add try catch to use this

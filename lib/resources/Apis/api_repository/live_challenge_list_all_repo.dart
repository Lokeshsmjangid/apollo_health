...
Future<LiveChallengeListModel> liveChallengeAllListApi() async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return LiveChallengeListModel.fromJson({});
  }
  try{
    String url = ApiUrls.liveChallengeListAllUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return LiveChallengeListModel.fromJson(data);
    } else {
  ...
  return LiveChallengeListModel.fromJson({}); // please add try catch to use this
  // return LiveChallengeListModel.fromJson(data); // please UnComment to print data and remove try catch
}
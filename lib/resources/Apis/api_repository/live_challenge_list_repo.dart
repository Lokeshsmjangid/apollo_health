...
Future<LiveChallengeModel> liveChallengeDataApi({id}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return LiveChallengeModel.fromJson({});
  }
  try{
    String url = ApiUrls.liveChallengeListUrl;
    final Map<String, dynamic> map = {
      'live_challenge_id':id
    };
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return LiveChallengeModel.fromJson(data);
    } else {
  ...
  return LiveChallengeModel.fromJson({}); // please add try catch to use this
  // return LiveChallengeModel.fromJson(data); // please UnComment to print data and remove try catch
}
...
Future<SettingsCustomizationModel> getSettingsCustomizationApi() async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return SettingsCustomizationModel.fromJson({});
  }
  try{
    String url = ApiUrls.settingsCustomizationUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return SettingsCustomizationModel.fromJson(data);
    } else {
  ...
  return SettingsCustomizationModel.fromJson({}); // please add try catch to use this
  // return SettingsCustomizationModel.fromJson(data); // please UnComment to print data and remove try catch
}
...
Future<LiveChallengeScoreModel> scoreLiveChallengesApi({liveChallengeId}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return LiveChallengeScoreModel.fromJson({});
  }

  try{
    String url = ApiUrls.scoreLiveChallengeUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return LiveChallengeScoreModel.fromJson(data);
    } else {
  ...
  return LiveChallengeScoreModel.fromJson({}); // please add try catch to use this
  // return LiveChallengeScoreModel.fromJson(data); // please UnComment to print data and remove try catch
}
...
Future<WheelForWellnessModel> wheelForWellnessApi() async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return WheelForWellnessModel.fromJson({});
  }
  try{
    String url = ApiUrls.wheelForWellnessUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return WheelForWellnessModel.fromJson(data);
    } else {
  ...
  return WheelForWellnessModel.fromJson({}); // please add try catch to use this

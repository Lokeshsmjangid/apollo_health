...
Future<WheelForWellnessResultModel> getWellNessResultApi({
  wellnessGameId,wellnessId,wheelPoint,stuck,stuckCount,answer,time,lives}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return WheelForWellnessResultModel.fromJson({});
  }
  try{
    String url = ApiUrls.wheelForWellnessResultUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return WheelForWellnessResultModel.fromJson(data);
    } else {
  ...
  return WheelForWellnessResultModel.fromJson({}); // please add try catch to use this

...
Future<StartMedLingoModel> startMedLingoApi() async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return StartMedLingoModel.fromJson({});
  }
  try{
    String url = ApiUrls.startMedLingoUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return StartMedLingoModel.fromJson(data);
    } else {
  ...
  return StartMedLingoModel.fromJson({}); // please add try catch to use this

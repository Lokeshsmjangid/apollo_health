...
Future<PlayRequestModel> playRequestApi({groupGameId, senderId}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return PlayRequestModel.fromJson({});
  }
  try{
    String url = ApiUrls.playRequestUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return PlayRequestModel.fromJson(data);
    } else {
  ...
  return PlayRequestModel.fromJson({}); // please add try catch to use this
  // return PlayRequestModel.fromJson(data); // please UnComment to print data and remove try catch
}
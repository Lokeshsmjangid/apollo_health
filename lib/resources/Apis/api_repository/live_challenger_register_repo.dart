...
Future<LiveChallengeRegisterModel> liveChallengeRegisterApi({liveChallengeId}) async {
  bool checkInternet = await hasInternetConnection();
  if (!checkInternet) { // checkInternet is false
    CustomSnackBar().showSnack(Get.context!,isSuccess: false,message: 'No Internet Connection');
    return LiveChallengeRegisterModel.fromJson({});
  }
  try{
    String url = ApiUrls.liveChallengeRegisterUrl;
    ...
    if (response.statusCode == 200) {
      log("\n response Start-->\n\n $data \n\n<--response End" );
      return LiveChallengeRegisterModel.fromJson(data);
    } else {
  ...
  return LiveChallengeRegisterModel.fromJson({}); // please add try catch to use this
